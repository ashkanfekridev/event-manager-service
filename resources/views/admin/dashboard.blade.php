...
<div class="grid" style="margin-top:18px">
    ...
    <section class="card"><div class="section-title"><h2>آخرین سفارش‌ها</h2></div><div class="table-wrap"><table class="table"><tr><th>خریدار</th><th>مبلغ</th><th>وضعیت</th></tr>@forelse($recentOrders as $order)<tr><td>{{ $order->customer_name }}</td><td>{{ number_format($order->total_amount) }}</td><td><span class="badge">{{ $order->status }}</span></td></tr>@empty<tr><td colspan="3">هنوز سفارشی ثبت نشده است.</td></tr>@endforelse</table></div></section>
</div>
@endsection
